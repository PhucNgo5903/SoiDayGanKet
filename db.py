...
# ===== 1. Lấy danh sách kỹ năng và loại yêu cầu hỗ trợ =====
cur.execute('SELECT id FROM app_skill')
skill_ids = [row[0] for row in cur.fetchall()]

cur.execute('SELECT id FROM app_assistancerequesttype')
assistance_type_ids = [row[0] for row in cur.fetchall()]

# ===== 2. Tạo dữ liệu mapping kỹ năng <-> loại yêu cầu hỗ trợ =====
# Mỗi loại yêu cầu hỗ trợ gắn với ngẫu nhiên 2-5 kỹ năng
mapping_records = []

for atype_id in assistance_type_ids:
    selected_skills = random.sample(skill_ids, random.randint(2, 5))
    for skill_id in selected_skills:
        mapping_records.append((skill_id, atype_id))
...
print(f"Đã chèn {len(mapping_records)} bản ghi vào bảng app_skillassistancerequesttype.")
